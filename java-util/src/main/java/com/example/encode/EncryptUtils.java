...
/**
 * 类EncryptUtils.java的实现描述：TODO 类实现描述
 * 
 * @author xupeng 2017年3月25日 上午10:46:54
 */
public class EncryptUtils {
    ...
    public static String getUniqueKey() {
        return encrypt(RandomStringUtils.randomNumeric(10) + System.currentTimeMillis(), SHA);
    }
    ...
    public static String desEncrypt(String input, String strkey) throws Exception {
        DESedeKeySpec dks = new DESedeKeySpec(input.getBytes("UTF-8"));
        SecretKeyFactory keyFactory = SecretKeyFactory.getInstance("DESede");
        SecretKey securekey = keyFactory.generateSecret(dks);

        Cipher cipher = Cipher.getInstance("DESede/ECB/PKCS5Padding");
        cipher.init(Cipher.ENCRYPT_MODE, securekey);
        byte[] b = cipher.doFinal(strkey.getBytes());
    ...
    public static String desDecrypt(String cipherText, String strkey) throws Exception {
        ...
        // --解密的key
        DESedeKeySpec dks = new DESedeKeySpec(strkey.getBytes("UTF-8"));
        SecretKeyFactory keyFactory = SecretKeyFactory.getInstance("DESede");
        SecretKey securekey = keyFactory.generateSecret(dks);
        // --Chipher对象解密
        Cipher cipher = Cipher.getInstance("DESede/ECB/PKCS5Padding");
